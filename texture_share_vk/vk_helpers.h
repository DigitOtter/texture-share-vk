...
class VkHelpers
{
	public:
		...
		static void ImmediateSubmit(VkDevice device, VkQueue queue, VkCommandBuffer command_buffer, const
		                            std::function<void(VkCommandBuffer command_buffer)> &f,
		...
		static void CmdClearColorImage(VkCommandBuffer command_buffer, VkImage image, const VkClearColorValue &color_value, VkImageLayout image_layout);
		static void CmdPipelineMemoryBarrierColorImage(VkCommandBuffer command_buffer, VkImage image,
		...
		static uint32_t GetMemoryType(VkPhysicalDevice physical_device, uint32_t bits, VkMemoryPropertyFlags properties, VkBool32 *memory_type_found = nullptr);
};
